# CJDNS support in Bitquantum Core

It is possible to run Bitquantum Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitquantum Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitquantum network and individual nodes.
...
before it will work with your Bitquantum Core node. This is described in steps
...
## Run Bitquantum Core with CJDNS

Once you are connected to the CJDNS network, the following Bitquantum Core
...
When enabled, this option tells Bitquantum Core that it is running in an
...
IPv6 local network. This helps Bitquantum Core perform better address management:
...
CJDNS support was added to Bitquantum Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitquantum-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitquantum/bitquantum/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bitquantum Core

There are several ways to see your CJDNS address in Bitquantum Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitquantum-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitquantum-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitquantum-cli help getnodeaddresses` for details.
