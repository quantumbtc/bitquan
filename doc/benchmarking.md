...
Bitquantum Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_bitquantum`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_bitquantum

After compiling bitquantum-core, the benchmarks can be run with:

    build/bin/bench_bitquantum
...
To monitor Bitquantum Core performance more in depth (like reindex or IBD): https://github.com/bitquantum-dev-tools/benchcoin
