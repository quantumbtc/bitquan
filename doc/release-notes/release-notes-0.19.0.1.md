Bitquantum Core version 0.19.0.1 is now available from:

  <https://bitquantumcore.org/bin/bitquantum-core-0.19.0.1/>
...
Bitquantum Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Bitquantum Core on unsupported systems.
...
to open the Bitquantum Core .dmg. This is due to new signing requirements
imposed by Apple, which the Bitquantum Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitquantum/bitquantum/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Bitquantum Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Bitquantum
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitquantumd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Bitquantum Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Bitquantum Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Bitquantum Core. (#14054)

- To eliminate well-known denial-of-service vectors in Bitquantum Core,
  ...
  This prevents Bitquantum Core from sending the BIP111 NODE_BLOOM service
  ...
  of Bitquantum Core and nodes that have manually enabled BIP37 support,
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Bitquantum Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Bitquantum Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Bitquantum Core will be bound to
  the standard port 51997 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Bitquantum address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Bitquantum Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Bitquantum AEAD (jonasschnelli)
...
- #15639 bitquantum-wallet tool: Drop libbitquantum_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in bitquantumd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitquantum-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitquantumd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitquantum_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitquantum to auto generated project files (sipsorcery)
...
- #15773 Add BitquantumTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitquantum output (practicalswift)
...
- #15896 feature_filelock, interface_bitquantum_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitquantum Core (luke-jr)
...
- #16277 Suppress output in test_bitquantum for expected errors (gertjaap)
...
- #16489 log: harmonize bitquantumd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitquantumVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitquantum.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBBITQUANTUM_SERVER (Bushstar)
...
- #16080 build/doc: update bitquantum_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitquantum.conf (fanquake)
...
- #16621 add default bitquantum.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitquantum-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitquantum.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitquantumd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitquantum/bitquantum/).
