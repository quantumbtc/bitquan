...
Bitquantum Core version 25.0 is now available from:

  <https://bitquantumcore.org/bin/bitquantum-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/bitquantum/bitquantum/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `bitquantum-cli` tool both take advantage of this, so
  for example:

```sh
bitquantum-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
bitquantum-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Bitquantum Core will now return an error at startup. (#25574)
...
before Bitquantum Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching bitquantumd with the configuration option
