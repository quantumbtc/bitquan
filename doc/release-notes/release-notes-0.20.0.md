...
Bitquantum Core version 0.20.0 is now available from:

  <https://bitquantumcore.org/bin/bitquantum-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Bitquantum
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Bitquantum Core on
unsupported systems.

From Bitquantum Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Bitquantum Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `BITQUANTUM_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Bitquantum Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bitquantum Core version 0.18.0.
...
- Testing or debugging of implementations of the Bitquantum P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bitquantum Core. Bitquantum Core logs debug messages
...
- OpenSSL is no longer used by Bitquantum Core.  (#17265)

- BIP70 support has been fully removed from Bitquantum Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitquantum/bitquantum/issues/16599),
  [PR #16702](https://github.com/bitquantum/bitquantum/pull/16702), and the
  `bitquantumd help` for more information.  This option is experimental and
...
- All custom settings configured when Bitquantum Core starts are now
...
- The "Start Bitquantum Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitquantumcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Bitquantum Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bitquantum Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitquantum-core/HWI) for
  signing.  Future versions of Bitquantum Core should support a GUI option
...
- Bitquantum Core's automatically-generated source code documentation is
  now available at https://doxygen.bitquantumcore.org.  (#17596)
...
- The `bitquantum-cli` utility used with the `-getinfo` parameter now
...
- The `bitquantum-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=51997` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `bitquantumstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitquantum-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBITQUANTUM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitquantum` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBITQUANTUM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitquantum-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitquantum schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitquantum.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitquantum` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitquantum-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitquantum-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bitquantum Core files (hebasto)
...
- #17119 Fix broken bitquantum-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitquantum-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitquantumcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitquantum-wallet (hel-o)
...
- #16115 On bitquantumd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitquantum-cli -getinfo (malevolent)
