 ...
 */
static const char* BITQUANTUM_PID_FILENAME = "bitquantumd.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", BITQUANTUM_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", BITQUANTUM_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-allowignoredconf", strprintf("For backwards compatibility, treat an unused %s file in the datadir as a warning, not an error.", BITQUANTUM_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-persistmempoolv1",
                   ...
                   ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BITQUANTUM_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", BITQUANTUM_CONF_FILENAME, BITQUANTUM_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-bind=<addr>[:<port>]", strprintf("Bind to given address and always listen on it (default: 0.0.0.0). Use [host]:port notation for IPv6. Onion/Tor support disabled.", defaultChainParams->GetDefaultPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u). Only mainnet supported.", defaultChainParams->GetDefaultPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if bitquantum is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitquantum is started while in a temporary directory.\n",
    ...
    */
    ...
    // Onion/Tor support disabled - no onion port needed
    const uint16_t default_bind_port_onion = 0;
    ...
    for (const std::string& bind_arg : args.GetArgs("-bind")) {
        ...
        if (index == std::string::npos) {
            ...
            if (network_type == "onion") {
                // Onion/Tor support disabled
                LogInfo("Onion binding requested but disabled - ignoring");
                continue;
            }
        }
    ...
    if (!connOptions.onion_binds.empty()) {
        ...
        // Onion/Tor support disabled - no onion service target
        LogInfo("Onion service disabled - skipping onion binding");
    }

    if (false) { // Force disable onion listening
