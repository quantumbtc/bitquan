                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<BitquantumAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = BitquantumUnits::formatWithUnit(unit, expected_balance, false, BitquantumUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_bitquantum-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_bitquantum-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_bitquantum-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    BitquantumAmountField* amountInput = receiveCoinsDialog.findChild<BitquantumAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("bitquantum:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_bitquantum-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
