...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<BitquantumUnit>("BitquantumUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitquantum_de.qm (shortcut "de" needs to be defined in bitquantum.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitquantum_de_DE.qm (shortcut "de_DE" needs to be defined in bitquantum.qrc)
...
static const char* qt_argv = "bitquantum-qt";

BitquantumApplication::BitquantumApplication()
...
void BitquantumApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitquantumApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitquantumGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitquantumApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitquantumApplication::createOptionsModel(bool resetSettings)
{
...
void BitquantumApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitquantumGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitquantumGUI::quitRequested, this, &BitquantumApplication::requestShutdown);
...
void BitquantumApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitquantumApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitquantumApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitquantumApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitquantumApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitquantumApplication::handleRunawayException);
    connect(this, &BitquantumApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitquantumApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitquantumApplication::parameterSetup()
{
...
void BitquantumApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitquantumApplication::requestInitialize()
{
...
void BitquantumApplication::requestShutdown()
{
...
void BitquantumApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitquantum: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitquantumGUI::handlePaymentRequest);
            connect(window, &BitquantumGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitquantumApplication::handleRunawayException(const QString &message)
{
...
void BitquantumApplication::handleNonFatalException(const QString& message)
{
...
WId BitquantumApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitquantumApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitquantumGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitquantum);
    Q_INIT_RESOURCE(bitquantum_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BITQUANTUM_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitquantum-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitquantum-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitquantum.conf, determine network, switch to network specific
    ...
    // bitquantum: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
