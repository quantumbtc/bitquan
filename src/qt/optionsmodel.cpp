...
/** Call node.updateRwSetting() with Bitquantum 22.x workaround. */
...
//! Convert enabled/size values to bitquantum -prune setting.
...
//! Get pruning enabled value to show in GUI from bitquantum -prune setting.
...
//! Get pruning size value to show in GUI from bitquantum -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitquantumUnit")) {
        settings.setValue("DisplayBitquantumUnit", QVariant::fromValue(BitquantumUnit::BTQ));
    }
    QVariant unit = settings.value("DisplayBitquantumUnit");
    if (unit.canConvert<BitquantumUnit>()) {
        m_display_bitquantum_unit = unit.value<BitquantumUnit>();
    } else {
        m_display_bitquantum_unit = BitquantumUnit::BTQ;
        settings.setValue("DisplayBitquantumUnit", QVariant::fromValue(m_display_bitquantum_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitquantum.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitquantum_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitquantumUnit>() == m_display_bitquantum_unit) return;
    m_display_bitquantum_unit = new_unit.value<BitquantumUnit>();
    QSettings settings;
    settings.setValue("DisplayBitquantumUnit", QVariant::fromValue(m_display_bitquantum_unit));
    Q_EMIT displayUnitChanged(m_display_bitquantum_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/bitquantum-core/gui/issues/567).
