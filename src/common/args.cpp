...
const char * const BITQUANTUM_CONF_FILENAME = "bitquantum.conf";
const char * const BITQUANTUM_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitquantum-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITQUANTUM_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Bitquantum
    //   new: C:\Users\Username\AppData\Local\Bitquantum
    // macOS: ~/Library/Application Support/Bitquantum
    // Unix-like: ~/.bitquantum
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Bitquantum";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bitquantum";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Bitquantum";
#else
    // Unix-like
    return pathRet / ".bitquantum";
#endif
#endif
}
