...
static void SetupBitquantumTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitquantum-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitquantum-tx tool is used for creating and modifying bitquantum transactions.\n\n"
                "bitquantum-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitquantum transaction, or with \"-create [commands]\" to create a hex-encoded bitquantum transaction.\n"
                "\n"
                "Usage: bitquantum-tx [options] <hex-tx> [commands]\n"
                "or:    bitquantum-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitquantum transaction
