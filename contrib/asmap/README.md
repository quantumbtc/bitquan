...
be used in Bitquantum Core with the `-asmap` option.
...
to binary before being used with Bitquantum Core.
...
the `bitquantum-cli getnodeaddresses` command.
...
```
bitquantum-cli getnodeaddresses 0 > addrs.json
```
