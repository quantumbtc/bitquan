#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Bitquantum Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitquantum-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitquantum-cli', help='bitquantum-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitquantum address to which the faucet should send')
...
parser.add_argument('bitquantum_cli_args', nargs='*', help='Arguments to pass on to bitquantum-cli (default: -signet)')

args = parser.parse_args()

if args.bitquantum_cli_args == []:
    args.bitquantum_cli_args = ['-signet']


def bitquantum_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitquantum_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitquantum_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitquantum_cli(['getnewaddress', 'faucet', 'bech32'])
