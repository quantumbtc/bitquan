...
"""Script for verifying Bitquantum Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitquantumcore.org and bitquantum.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/bitquantum-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://bitquantumcore.org"
HOST2 = "https://bitquantum.org"
VERSIONPREFIX = "bitquantum-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Bitquantum community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"bitquantum_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by bitquantumcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the bitquantum release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes bitquantum.org lags behind bitquantumcore.org.)')
    )
