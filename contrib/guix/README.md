# Bootstrappable Bitquantum Core Builds

This directory contains the files necessary to perform bootstrappable Bitquantum
Core builds.
...
To build Bitquantum Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bitquantum Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitquantum-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitquantum-core/guix.sigs).
