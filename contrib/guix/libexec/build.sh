#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Bitquantum Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/bitquantum-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Bitquantum Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Bitquantum Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example bitquantum.conf file. if contrib/devtools/gen-bitquantum-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/bitquantum.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Bitquantum-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
