...
# Copyright 2016-present The Bitquantum Core developers
...
class ToolUtils(BitquantumTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_bitquantum_tx()
        self.skip_if_no_bitquantum_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "bitquantum-util-test.json", encoding="utf8") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./bitquantum-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./bitquantum-tx":
