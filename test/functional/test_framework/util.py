...
def assert_fee_amount(fee, tx_size, feerate_BTQ_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_BTQ_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s BTQ too low! (Should be %s BTQ)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTQ_kvB)
    if fee > high_fee:
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `BitquantumTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/bitquantumcore/bitquantum /issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitquantum.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Bitquantum"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Bitquantum"
        else:
            datadir = temp_dir / ".bitquantum"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitquantum.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitquantum.conf")):
        with open(os.path.join(datadir, "bitquantum.conf"), 'r', encoding='utf8') as f:
            for line in f:
